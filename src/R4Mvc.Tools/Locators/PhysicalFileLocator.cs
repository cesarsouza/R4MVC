...
namespace R4Mvc.Tools.Locators
{
    public class PhysicalFileLocator : IFileLocator
    {
        ...
        public string[] GetDirectories(string parentPath, string filter = "*", bool recurse = false)
            => Directory.GetDirectories(parentPath, filter, recurse ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly);
